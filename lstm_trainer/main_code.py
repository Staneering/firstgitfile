...
# Data
dat = pd.read_csv('REAL POPULATION.csv')
fixed_col = dat['perc'].apply(conv_float)

dat['perc'].update(fixed_col)

#fixed_zero = dat['perc'].apply(conv_zero)

dat['perc'].astype('float')

print(dat['perc'].to_string())

print('dat is')
print(dat.head().to_string())
...
future.predict_future(X_min = X_min, X_max = X_max, numeric_colname = 'perc', timesteps_to_predict = 180, return_future = True)
